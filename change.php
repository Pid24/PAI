...
$kmps = query("SELECT * FROM program_mbkm WHERE id = $id")[0];
    ...
    <section id="form">
      <div class="container">
        <div class="row text-center mb-3">
          <div class="col1">
            <h2>Change Nama Kampus</h2>
          </div>
        </div>
        <div class="row justify-content-center">
          <form action="" method="post">
            <input type="hidden" name="id" value="<?= $kmps["id"]; ?>">
            <div class="mb-3">
              <label for="nama_kampus" class="form-label">Nama Kampus</label>
              <input type="text" class="form-control" id="nama_kampus" aria-describedby="name" name="nama_kampus" required value="<?= $kmps["nama_kampus"] ?>"/>
            </div>
            <button type="submit" class="btn btn-success btn-submit" name="submit">Submit</button>
          </form>
        </div>
      </div>
    </section>
